...
class Comp extends PureComponent {
  ...
  render () {
    const { prefixCls, taxHeader, taxPurchase, taxSales } = this.props
    const dataTr = this.state.taxDataList.map((item, index) => {
      return (
        <tr className={`${prefixCls}-dataBox-data-dataTr`} key={item.key}>
          <td>{item.name}</td>
          <td>{item.type}</td>
          <td>{item.unit}</td>
          <td>{item.number}</td>
      ...
      <div className={`${prefixCls} ${prefixCls}-con`}>
        ...
        <div className={`${prefixCls}-footer`}>
          ...
          <span style={{ marginLeft: 8 }}>
            <Radio.Group value={this.state.ticketType} onChange={this.radioChange}>
